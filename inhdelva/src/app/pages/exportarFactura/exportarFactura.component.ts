...
@Component({
  ...
  templateUrl: './exportarFactura.component.html',
  styleUrls: ['./exportarFactura.component.scss']
})
export class ExportarFacturaComponent implements OnInit {
  ...
  async generarFactura(id, fechaInicio, fechaFin, contratoid, medidorId) {
    return await new Promise(async (res, rej) => {
      await this.facturaService.getDetalleFactura(
        id,
  ...
  async generarPDF() {
    ...
    html2canvas(div, options).then(async (canvas) => {
      ...
      const bufferX = 5;
      const bufferY = 5;
      const imgProps = await (<any>doc).getImageProperties(img);
      const pdfWidth = doc.internal.pageSize.getWidth() - 2 * bufferX;
      const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;

      await (doc as any).addImage(img, 'PNG', bufferX, bufferY, pdfWidth, pdfHeight, undefined, 'FAST');
      ...
      if (this.isVisibleAnexo) {
        ...
        html2canvas(anexo, options).then(async (canvas) => {
          var img = canvas.toDataURL("image/PNG");
          // Add image Canvas to PDF
          const bufferX = 5;
          const bufferY = 5;
